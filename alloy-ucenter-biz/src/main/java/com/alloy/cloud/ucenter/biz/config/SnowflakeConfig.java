        ...
        if (instance == null) {
            synchronized (SnowflakeIdWorker.class) {
                if (instance == null) {
                    ...
                    Long workerId = stringRedisTemplate.opsForValue().increment("workerId:"+serverName, 1); //不存在则返回1，存在则+1并返回
                    Long dataCenterId = stringRedisTemplate.opsForValue().increment("dataCenterId:"+serverName, 1);
